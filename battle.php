...
// 2. 取得したダンジョンIDを使って floor_monsters を検索
...
if ($player_current_hp > 0) {
    ...
    // --- ▼▼▼ アイテムドロップ処理 (スタック・スロット・不可フラグ対応) ▼▼▼ ---
    
    $base_drop_rate = $monster_data['base_drop_rate'] ?? 0.0; 
        ...
        if (!empty($possible_drops)) {
            ...
            if ($total_weight > 0) {
                ...
                if ($dropped_item_id !== null) {
                    ...
                    $item_template_stmt->bindValue(':item_id', $dropped_item_id, PDO::PARAM_INT);
                    ...
                    if ($item_template) {
                        ...
                        $not_stackable = isset($item_template['not_stackable']) ? (int)$item_template['not_stackable'] : 0;

                        $pdo->beginTransaction(); 
                        
                        // 現在のスロット数
                        $stmt_count = $pdo->prepare("SELECT COUNT(inventory_id) FROM player_inventory WHERE player_id = :player_id");
                        $stmt_count->bindValue(':player_id', $_SESSION['player_id'], PDO::PARAM_INT);
                        $stmt_count->execute();
                        ...
                        if ($item_type === 'equipment') {
                            if ($current_slots >= MAX_INVENTORY_SLOTS) {
                                $battle_log[] = 'しかし、所持品がいっぱいで「' . $item_template['name'] . '」は手に入らなかった...';
                                $pdo->rollBack();
                            } else {
                                // オプション生成
                                ...
                                $sql = "INSERT INTO player_inventory (player_id, item_id, quantity, option_1_stat, option_1_value, option_2_stat, option_2_value, option_3_stat, option_3_value, option_4_stat, option_4_value, option_5_stat, option_5_value) 
                                        VALUES (:player_id, :item_id, 1, :o1s, :o1v, :o2s, :o2v, :o3s, :o3v, :o4s, :o4v, :o5s, :o5v)";
                            ...
                            $existing_slot = false;
                            // スタック可能(not_stackable=0)なら、空きスロットを探す
                            if ($not_stackable === 0) {
                                ...
                                $stmt_find_stack->bindValue(':player_id', $_SESSION['player_id'], PDO::PARAM_INT);
                            ...
                            if ($existing_slot) {
                                ...
                                $stmt_upd = $pdo->prepare("UPDATE player_inventory SET quantity = :qty WHERE inventory_id = :id");
                                $stmt_upd->bindValue(':qty', $new_qty, PDO::PARAM_INT);
                                $stmt_upd->bindValue(':id', $existing_slot['inventory_id'], PDO::PARAM_INT);
                                $stmt_upd->execute();
                                $battle_log[] = $item_template['name'] . 'を手に入れた！';
                                $pdo->commit();
                                ...
                                if ($current_slots >= MAX_INVENTORY_SLOTS) {
                                    ...
                                    $stmt_ins->bindValue(':player_id', $_SESSION['player_id'], PDO::PARAM_INT);
